...
// --- 状态数据 ---
const activeView = ref('transactions') // 'transactions' | 'settings'
const settingsTab = ref('accounts')    // 'accounts' | 'categories'

const accounts = ref([])
const categories = ref([])
const transactions = ref([])
const selectedAccount = ref(null) // 左侧栏选中的账户

// 筛选状态
const filterMode = ref('MONTH') // 'ALL', 'YEAR', 'MONTH', 'WEEK'
const cursorDate = ref(new Date()) // 当前选中的时间锚点
...
const submitTransaction = async () => {
  ...
  const payload = {
    ...
    fund_account_id: (form.value.type === 'EXPENSE' && form.value.fund_account_id) ? Number(form.value.fund_account_id) : null,
  ...
  if (res.ok) { 
    form.value.amount = ''; form.value.note = ''; form.value.tag = ''; form.value.fund_account_id = '';
    showRecordModal.value = false; await fetchData() 
  } else { alert('保存失败') }
}

const deleteTransaction = async (id) => { if (confirm("确定删除这条记录吗？")) { await fetch(`/api/transactions/${id}`, { method: 'DELETE' }); await fetchData() } }
...
const submitAccount = async () => {
  if (!accountForm.value.name) return alert("请输入账户名称")
...
const deleteAccount = async (id) => { if (confirm("⚠️ 警告：删除账户将同时删除该账户下的所有交易记录！")) { await fetch(`/api/accounts/${id}`, { method: 'DELETE' }); if (selectedAccount.value?.id === id) selectedAccount.value = null; await fetchData() } }
...
const dateRange = computed(() => {
  ...
  if (filterMode.value === 'YEAR') {
    ...
    start = new Date(d); start.setHours(0,0,0,0); start.setDate(d.getDate() - day + 1)
...
const dateLabel = computed(() => {
  ...
  if (filterMode.value === 'WEEK') {
    ...
    return `${start.getMonth()+1}.${start.getDate()} ~ ${end.getMonth()+1}.${end.getDate()} (周)`
  }
  return ''
})
...
const filteredTransactions = computed(() => {
  ...
  if (selectedAccount.value) {
    const aid = selectedAccount.value.id
    list = list.filter(t => t.account_id === aid || t.target_account_id === aid)
  }
...
// 顶部总资产 (始终显示当前账户余额，不受日期筛选影响，因为余额是累计的)
const assetStats = computed(() => {
  let a = 0, l = 0
  accounts.value.forEach(acc => {
    if (acc.balance >= 0) a += acc.balance
    else l += acc.balance
  })
...
<template>
  <div class="app-layout">
    ...
    <div class="sidebar">
      <div class="logo-area">
        <span class="logo-icon">💰</span> <span style="font-weight: bold;">我的账本</span>
      </div>
      <div class="nav-item" :class="{ active: activeView === 'transactions' && !selectedAccount }" @click="activeView = 'transactions'; selectedAccount = null">
        <span class="icon">📂</span> 所有交易
      </div>
      <div class="account-group" v-for="(group, type) in groupedAccounts" :key="type">
        <div class="group-header">
          <span>{{ type }}</span><span class="group-total">¥{{ group.total.toFixed(2) }}</span>
        </div>
        <div class="nav-item sub-item" v-for="acc in group.accounts" :key="acc.id" 
          ...
          <span class="acc-balance" :class="{ 'text-red': acc.balance < 0 }">{{ acc.balance.toFixed(2) }}</span>
        </div>
      </div>
      <div class="spacer"></div>
      <div class="nav-item settings-btn" :class="{ active: activeView === 'settings' }" @click="activeView = 'settings'">
        ...
        <div class="table-container">
          <div class="filter-bar">
            <div class="left-tools">
              <span class="current-view">{{ selectedAccount ? selectedAccount.name : '所有账户' }}</span>
            </div>
            
            <div class="date-tools">
              <div class="mode-switch">
                <button :class="{active: filterMode==='ALL'}" @click="filterMode='ALL'">全部</button>
                <button :class="{active: filterMode==='YEAR'}" @click="filterMode='YEAR'">年</button>
                <button :class="{active: filterMode==='MONTH'}" @click="filterMode='MONTH'">月</button>
                <button :class="{active: filterMode==='WEEK'}" @click="filterMode='WEEK'">周</button>
              </div>
              
              <div class="date-nav" v-if="filterMode !== 'ALL'">
                <button class="nav-btn" @click="shiftDate(-1)">◀</button>
                <span class="date-label">{{ dateLabel }}</span>
                <button class="nav-btn" @click="shiftDate(1)">▶</button>
                <button class="nav-btn today" @click="cursorDate=new Date()">今</button>
              </div>
            </div>
          </div>
          ...
          <table>
            <thead>
              <tr><th width="120">日期</th><th>分类</th><th class="text-right">流入</th><th class="text-right">流出</th><th>账户</th><th>备注/标签</th><th width="50"></th></tr>
            </thead>
            <tbody>
          ...
          <div class="table-footer" v-if="filteredTransactions.length > 0">
            <span>{{ dateLabel }} 合计：</span>
            <span class="stat-pill income">收入: {{ periodStats.income }}</span>
            <span class="stat-pill expense">支出: {{ periodStats.expense }}</span>
            <span class="stat-pill balance">结余: {{ periodStats.balance }}</span>
          </div>
        </div>
      </div>
        ...
        <div v-if="settingsTab === 'accounts'" class="settings-panel"><div class="settings-inner">
          ...
          <div class="account-list">
            ...
            <div class="account-row" v-for="acc in accounts" :key="acc.id">
              ...
              <span style="flex:1"><span class="badge">{{ acc.type }}</span></span><span class="text-right bold" style="flex:1">{{ acc.balance }}</span>
    ...
    <div class="modal-overlay" v-if="showRecordModal" @click.self="showRecordModal = false"><div class="modal-card">
        ...
        <div class="modal-form">
          ...
          <div class="row">
            <div style="flex:1"><select v-model="form.account_id"><option value="" disabled>{{form.type==='TRANSFER'?'转出账户':'记账账户'}}</option><option v-for="acc in accounts" :key="acc.id" :value="acc.id">{{ acc.name }}</option></select></div>
            ...
            <div v-if="form.type==='EXPENSE'" style="flex:1"><select v-model="form.fund_account_id" style="color:#2c3e50;border-color:#3498db"><option value="">默认 (余额支付)</option><optgroup label="实际扣款账户"><option v-for="acc in accounts.filter(a=>a.id!==form.account_id)" :key="acc.id" :value="acc.id">扣: {{ acc.name }}</option></optgroup></select></div>
          </div>
...
.nav-btn { border: 1px solid #ddd; background: white; border-radius: 4px; cursor: pointer; padding: 2px 8px; font-size: 12px; }
.nav-btn:hover { background: #f9f9f9; }
.nav-btn.today { font-weight: bold; color: #3498db; }
...
.stat-pill.income { background: #eafaf1; color: #27ae60; }
.stat-pill.expense { background: #fdedec; color: #e74c3c; }
...
.btn-sm.primary { background: #3498db; color: white; } .btn-sm.danger { background: #fff0f0; color: #e74c3c; }
...
.node-content.parent.income { border-left: 4px solid #27ae60; } .node-content.parent:not(.income) { border-left: 4px solid #c0392b; }
...
.tag-chip { background: #f0f0f0; color: #666; padding: 4px 12px; border-radius: 20px; font-size: 12px; cursor: pointer; border: 1px solid transparent; }
.tag-chip:hover { background: #e0e0e0; } .tag-chip.active { background: #e8f4fc; color: #3498db; border-color: #3498db; font-weight: bold; }
...
.logo-area { padding: 20px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid #eaeaea; }
